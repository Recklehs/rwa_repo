...
DB_USER="rwa"
DB_NAME="rwa"
SQL_FILE="${ROOT_DIR}/experiment/sql/migrations/V902__observability_extensions.sql"
...
echo "[obs] applying ${SQL_FILE}"
"${COMPOSE_CMD[@]}" exec -T postgres psql -v ON_ERROR_STOP=1 -U "${DB_USER}" -d "${DB_NAME}" -f - < "${SQL_FILE}"

echo "[obs] restarting postgres to ensure shared_preload_libraries changes are active"
